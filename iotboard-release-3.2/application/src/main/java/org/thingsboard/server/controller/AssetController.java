...
import org.thingsboard.server.common.data.asset.AssetCount;
...
public class AssetController extends BaseController {
    ...
    @PreAuthorize("hasAuthority('TENANT_ADMIN')")
    @RequestMapping(value = "/tenant/asset/count", method = RequestMethod.GET)
    @ResponseBody
    public AssetCount getAssetCount() throws ThingsboardException {
            TenantId tenantId = getCurrentUser().getTenantId();
            AssetCount response = new AssetCount();
            PageLink pageLink = createPageLink(1000, 0, null, null, null);
            PageData<AssetInfo> data = checkNotNull(assetService.findAssetInfosByTenantId(tenantId, pageLink));
            int available = 0;
            int assigned = 0;
            long total = data.getTotalElements();
            for(AssetInfo assetInfo : data.getData()){
                if (assetInfo.getCustomerTitle() == null) {  available++; }
                else { assigned++; }
            }
