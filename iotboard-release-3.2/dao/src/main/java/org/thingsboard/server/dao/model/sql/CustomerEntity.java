...
public final class CustomerEntity extends BaseSqlEntity<Customer> implements SearchTextEntity<Customer> {
    ...
    @Column(name = ModelConstants.STARTTIMEMS_PROPERTY)
    private long startTimeMs;
    
    @Column(name = ModelConstants.ENDTIMEMS_PROPERTY)
    private long endTimeMs;

    @Column(name = ModelConstants.NATIONALID_PROPERTY, unique = true)
    private String nationalid;

    @Column(name = ModelConstants.PASSPORTID_PROPERTY, unique = true)
    private String passportid;

    @Column(name = ModelConstants.PHONE_NUMBER_PROPERTY)
    private String phonenumber;

    @Column(name = ModelConstants.ROOM_STATUS)
    private String status;

    @Column(name = ModelConstants.ROOM_HOURS)
    ...
    public Customer toData() {
        ...
        customer.setZip(zip);
        customer.setStartTimeMs(startTimeMs);
        customer.setEndTimeMs(endTimeMs);
        customer.setNational_ID(nationalid);
        customer.setPassport_ID(passportid);
        customer.setEmail(email);
        customer.setAdditionalInfo(additionalInfo);
        customer.setPhone_number(phonenumber);
        customer.setStatus(startTimeMs, endTimeMs);
