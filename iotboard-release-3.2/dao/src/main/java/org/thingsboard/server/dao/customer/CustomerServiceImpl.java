...
public class CustomerServiceImpl extends AbstractEntityService implements CustomerService {
    ...
    // Custom Methods
    @Override
    public Optional<Customer> findCustomerByTenantIdAndNationalId(TenantId tenantId, String nationalid) {
        log.trace("Executing findCustomerByTenantIdAndTitle [{}] [{}]", tenantId, nationalid);
        validateId(tenantId, INCORRECT_TENANT_ID + tenantId);
        return customerDao.findCustomersByTenantIdAndNationalid(tenantId.getId(), nationalid);
    }

    @Override
    public Optional<Customer> findCustomerByTenantIdAndPassportId(TenantId tenantId, String passportid) {
        log.trace("Executing findCustomerByTenantIdAndTitle [{}] [{}]", tenantId, passportid);
        validateId(tenantId, INCORRECT_TENANT_ID + tenantId);
        return customerDao.findCustomersByTenantIdAndPassportid(tenantId.getId(), passportid);
    }

    @Override
    public Optional<Customer> findCustomerByTenantIdAndPhoneNumber(TenantId tenantId, String phone_number) {
        log.trace("Executing findCustomerByTenantIdAndTitle [{}] [{}]", tenantId, phone_number);
        validateId(tenantId, INCORRECT_TENANT_ID + tenantId);
        return customerDao.findCustomersByTenantIdAndPhoneNumber(tenantId.getId(), phone_number);
    }
    ...
    private DataValidator<Customer> customerValidator =
            new DataValidator<Customer>() {
                ...
                protected void validateCreate(TenantId tenantId, Customer customer) {
                    ...
                    if (customer.getNational_ID() != null) {
                        customerDao.findCustomersByTenantIdAndNationalid(customer.getTenantId().getId(), customer.getNational_ID()).ifPresent(
                                c -> {
                                    throw new DataValidationException("Customer with such NationalId already exists!");
                                }
                        );
                    }

                    if(customer.getPassport_ID() != null) {
                        customerDao.findCustomersByTenantIdAndPassportid(customer.getTenantId().getId(), customer.getPassport_ID()).ifPresent(
                                c -> {
                                    throw new DataValidationException("Customer with such PassportId already exists!");
                                }
                        );
                    }

                    if(customer.getPhone_number() != null) {
                        customerDao.findCustomersByTenantIdAndPhoneNumber(customer.getTenantId().getId(), customer.getPhone_number()).ifPresent(
                                c -> {
                                    throw new DataValidationException("Customer with such PhoneNumber already exists!");
                                }
                        );
                    }
                }
